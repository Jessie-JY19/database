...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int p = InnerNode.numLessThanEqual(key, this.keys);
        keys.add(p, key);
        rids.add(p, rid);
        ...
        List<DataBox> rightKeys = keys.subList(d, n);
        List<RecordId> rightRids = rids.subList(d, n);
        DataBox midKey = keys.get(d);
        LeafNode newNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids,
                this.rightSibling, treeContext);
        this.keys = leftKeys;
        this.rids = leftRids;
        long pageNum = newNode.page.getPageNum();
        this.rightSibling = Optional.of(pageNum);
        sync();
        return Optional.of(new Pair<>(midKey, pageNum));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int n = metadata.getOrder() * 2;
        int lim = (int) Math.floor(n * fillFactor);
        while (keys.size() <= lim) {
            if (!data.hasNext()) {
                sync();
                return Optional.empty();
        ...
        assert(keys.size() == lim + 1);
        List<DataBox> leftKeys = keys.subList(0, lim);
        List<RecordId> leftRids = rids.subList(0, lim);
        List<DataBox> rightKeys = keys.subList(lim, lim + 1);
        List<RecordId> rightRids = rids.subList(lim, lim + 1);
        DataBox midKey = keys.get(lim);
        LeafNode newNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids,
                this.rightSibling, treeContext);
        this.keys = leftKeys;
        this.rids = leftRids;
    ...
    Optional<LeafNode> getRightSibling() {
        long rPageNum = rightSibling.get();
        if (!rightSibling.isPresent() || rPageNum == -1) {
            return Optional.empty();
        }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        for (int i = 0; i < n; ++i) {
            keys.add(DataBox.fromBytes(buf, keySchema));
            rids.add(RecordId.fromBytes(buf));
        }
        Optional rightSib = Optional.of(rightSibling);
        LeafNode leaf = new LeafNode(metadata, bufferManager, page, keys, rids,
                rightSib, treeContext);
        return leaf;
    }
