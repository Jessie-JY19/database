...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        List<RecordId> rids = leaf.getRids();
        int n = keys.size();
        for (int i = 0; i < n; i++) {
            if (keys.get(i).compareTo(key) == 0) {
                RecordId rid = rids.get(i);
    ...
    public Iterator<RecordId> scanAll() {
        ...
        BPlusTreeIterator bIter = new BPlusTreeIterator(leftLeaf, leftLeaf.scanAll());
        return bIter;
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
        LeafNode leftLeaf = root.get(key);
        BPlusTreeIterator bIter = new BPlusTreeIterator(leftLeaf, leftLeaf.scanGreaterEqual(key));
        return bIter;
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> rv = this.root.put(key, rid);
        if (rv.isEmpty())
            return;
        Pair<DataBox, Long> input = rv.get();
        List<DataBox> newKeys = new ArrayList<>();
        List<Long> newChildren = new ArrayList<>();
        newKeys.add(input.getFirst());
        newChildren.add(root.getPage().getPageNum());
        newChildren.add(input.getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> rv = this.root.bulkLoad(data, fillFactor);
            if (rv.isEmpty())
                return;
            Pair<DataBox, Long> input = rv.get();
            List<DataBox> newKeys = new ArrayList<>();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public void nextLeaf() {
            Optional<LeafNode> rightSib = leaf.getRightSibling();
            if (rightSib.isEmpty()) {
                this.leaf = null;
                this.iter = null;
            }
            else {
                this.leaf = rightSib.get();
        ...
        public RecordId next() {
            ...
            assert(leaf != null && iter != null && iter.hasNext());
            RecordId rv = iter.next();
            if (!iter.hasNext())
                nextLeaf();
            return rv;
        }
