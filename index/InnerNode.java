...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int p = numLessThanEqual(key, this.keys);
        BPlusNode child = this.getChild(p);
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        // Q2(2)
        BPlusNode leftChild = this.getChild(0);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int p = numLessThanEqual(key, this.keys);
        BPlusNode child = this.getChild(p);
        Optional<Pair<DataBox, Long>> input = child.put(key, rid);
        if (input.isEmpty())
            return Optional.empty();

        Pair<DataBox, Long> pair = input.get();
        this.keys.add(p, pair.getFirst());
        this.children.add(p + 1, pair.getSecond());
        ...
        DataBox midKey = this.keys.get(d);
        List<DataBox> rightKeys = this.keys.subList(d+1, n);
        List<Long> rightChildren = this.children.subList(d + 1, 2 * d + 2);

        InnerNode newNode = new InnerNode(this.metadata, this.bufferManager, rightKeys, rightChildren, this.treeContext);

        this.keys = leftKeys;
        this.children = leftChildren;
        sync();

        return Optional.of(new Pair<>(midKey, newNode.getPage().getPageNum()));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (keys.size() <= n) {
            ...
            Optional<Pair<DataBox, Long>> rv = rightChild.bulkLoad(data, fillFactor);
            if (rv.isEmpty()) {
                sync();
                return Optional.empty();
            }
            Pair<DataBox, Long> input = rv.get();
            keys.add(input.getFirst());
            children.add(input.getSecond());
        }
