...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        while (records.hasNext()) {
            recs.add(records.next());
        }
        Collections.sort(recs, comparator);
        Run run = makeRun(recs);
        return run;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (runIters.get(j).hasNext()) {
                Pair newP = new Pair(runIters.get(j).next(), j);
                queue.add(newP);
            }
            mergedRun.add(r);
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        // TODO(proj3_part1): implement
        List<Run> passRuns = new ArrayList<>();
        int size = numBuffers - 1;
        int n = runs.size();
        for (int i = 0; i < n; i += size) {
            passRuns.add(mergeSortedRuns(runs.subList(i, i + size)));
        }
        return passRuns;
    }
    ...
    public Run sort() {
        ...
        while (runs.size() > 1) {
            List<Run> merged = mergePass(runs);
            runs = (ArrayList) merged;
        }
        return runs.size() > 0 ? runs.get(0) : makeRun();
    }
