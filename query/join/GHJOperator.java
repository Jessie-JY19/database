...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        // T(proj3_part1): implement the partitioning logic
        ...
        // to get a hash value.
        int colInd = left ? getLeftColumnIndex() : getRightColumnIndex();
        for (Record r : records) {
            DataBox colVal = r.getValue(colInd);
            int hash = HashFunc.hashDataBox(colVal, pass);
            int partNum = hash % partitions.length;
            if (partNum < 0) {
                partNum += partitions.length;
            }
            partitions[partNum].add(r);
        }
        return;
    }
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record br : buildRecords) {
            DataBox bjv = br.getValue(buildColumnIndex);
            if (!hashTable.containsKey(bjv)) {
                hashTable.put(bjv, new ArrayList<>());
            }
            hashTable.get(bjv).add(br);
        }

        for (Record pr : probeRecords) {
            DataBox pjv = pr.getValue(probeColumnIndex);
            if (hashTable.containsKey(pjv)) {
                for (Record br : hashTable.get(pjv)) {
                    Record jr = br.concat(pr);
                    this.joinedRecords.add(jr);
                }
            }
        }
        return;
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        int l = 39;
        for (int i = 0; i < l; i++) {
            Record lr = createRecord(7);
            leftRecords.add(lr);
        }
        Record r1 = createRecord(13);
        Record r2 = createRecord(19);
        rightRecords.add(r1);
        rightRecords.add(r2);
