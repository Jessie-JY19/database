...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int i : indices) {
            SelectPredicate sel = selectPredicates.get(i);
            IndexScanOperator op = new IndexScanOperator(transaction, table, sel.column, sel.operator, sel.value);
            int cost = op.estimateIOCost();
            if (cost < minCost) {
                minCost = cost;
                minOp = op;
                exclude = i;
        ...
        minOp = addEligibleSelections(minOp, exclude);
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        Set<Set<String>> prevKeys = prevMap.keySet();
        for (Set set: prevKeys) {
            for (JoinPredicate jp : this.joinPredicates) {
                ...
                QueryOperator qm = minCostJoinType(ql, qr, leftCol, rightCol);
                Set<String> newKey = new HashSet<>();
                newKey.addAll(set);
                newKey.add(leftTable);
                newKey.add(rightTable);
                result.put(newKey, qm);
            }
        }
        return result;
    }
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> p1 = new HashMap<>();
        Map<Set<String>, QueryOperator> cur;
        ...
        cur = p1;
        while (cur.size() > 1) {
            cur = minCostJoins(cur, p1);
        }
        finalOperator = minCostOperator(cur);
        this.addGroupBy();
        this.addProject();
        this.addSort();
        this.addLimit();
